...
export class SitemapEnhancedCrawlAgent extends BaseADIAgent {
  ...
  private readonly MAX_SITEMAP_INDEXES_TO_PROCESS = 1 // Extremely conservative: Only process 1 sitemap index
  private readonly MAX_CONTENT_SITEMAPS_TO_PROCESS = 2 // Extremely conservative: Only process 2 content sitemaps
  private readonly MAX_SITEMAP_URLS_TO_PROCESS = 20 // Extremely conservative: Max URLs to process per sitemap
  private readonly MAX_TOTAL_URLS_DISCOVERED = 1500 // Extremely conservative: Stop after discovering 1.5k URLs total
  ...
  private async discoverAndParseSitemap(websiteUrl: string): Promise<SitemapData | null> {
    const startTime = Date.now()
    const MAX_SITEMAP_PROCESSING_TIME = 3 * 60 * 1000 // 3 minutes timeout for sitemap processing (extremely conservative)
    ...
    while (indexQueue.length > 0 && this.sitemapIndexesFetched < this.MAX_SITEMAP_INDEXES_TO_PROCESS) {
      ...
      try {
        ...
        if (sitemapData && sitemapData.urls.length > 0) {
          ...
          // Extremely conservative smart exit if we have sufficient URLs from index exploration
          if (allUrls.length > 800) {
    ...
    while (contentQueue.length > 0 && 
      ...
      try {
        ...
        if (sitemapData && sitemapData.urls.length > 0) {
          ...
          // Extremely conservative smart exit when we have sufficient URLs
          if (sitemapData.urls.length > 500 || allUrls.length > 1200) {
